...
export default function ButtonMint() {
    ...
    const contractCall = async () => {
        try {
            const networkId = String(await web3.eth.net.getId()) as ObjectKey;
            const deployedNetwork = LabCoin.networks[networkId];
            const abi = LabCoin.abi as AbiItem[];
            labCoinInstance = new web3.eth.Contract(
                abi,
    ...
    return (
        <Button variant="contained" color="secondary" onClick={mint}>
            Clique aqui e pegue algumas LabCoins
        </Button>
    );
}
